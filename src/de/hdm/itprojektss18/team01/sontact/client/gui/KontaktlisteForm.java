...
public class KontaktlisteForm extends VerticalPanel {
	...
	// Setzen des Zugriffs auf das Asynchrone Service Interface
	...
	public KontaktlisteForm(Kontaktliste kl) {
		...
		/**
		 * Erstellung der Buttons und dem zuweisen zu einem CSS-Style.
		 */
		...
		// AddKontakt-Button instanziieren und dem Panel zuweisen
		...
		// ClickHandler fuer das Hinzufuegen eines Kontakts
		...
		// Styling des deleteTeilhaber-Buttons
		...
		// ClickHandler fuer das Loeschen einer Teilhaberschaft
		deleteTeilhaber.addClickHandler(new deleteTeilhaberClickHandler());
		BtnPanel.add(deleteTeilhaber);

		/**
		 * Prüfungs Methode ob der Nutzer der Owner des Kontaktes ist um dementsprechen das Label
	...
	public KontaktlisteForm(final Nutzer n) {
		...
		// Der TextBox einen KexDownHandler hinzufuegen für User freundlichen Umgang
			...
			if (x == true) {
				...
				if (nutzer.getId() != kl.getOwnerId()) {
				  ...
				  // Ist man Owner der Kontaktliste wird die Kontaktliste direkt geloescht.
					...
					// Zusaetzliche Pruefung ob es sich um eines der default Kontaktlisten handelt.
	...
	public class SpeichernKontaktlisteClickHandler implements ClickHandler {
		...
		public void onClick(ClickEvent event) {
			...
			// Pruefung on mindestens eine Name fuer die Kontaktliste angegeben wurde
			if (txtBox.getText() == "") {
				MessageBox.alertWidget("Hinweis", "Bitte geben Sie der Kontaktliste einen Namen");
				txtBox.getElement().getStyle().setBorderColor("red");
			} else {
				
				// TextBox wird ausgelesen und dadurch die Kontaktliste createt
	...
	/**
	 * ClickHandler um Kontakte einer Kontaktliste hinzuzufuegen.
	 */
	...
	private class updateKontaktlisteClickHandler implements ClickHandler {
		...
		public void onClick(ClickEvent event) {
			...
			// Hinzufuegen des Titels der Kontaktliste in den ContentHeader
			RootPanel.get("contentHeader").add(new HTML(kl.getTitel()
					+ " bearbeiten <image src='/images/edit.png' width='30px' height='30px' align='center'/></h2>"));
			
			// Abbrechen-Button intanziieren 
			...
			// ClickHandler fuer das Abbrechen 
			...
			if (kl.getTitel() == "Meine Kontakte" && kl.getOwnerId() == nutzer.getId()
				...
				Button saveBtn = new Button(
						"<image src='/images/save.png' width='20px' height='20px' align='center'/> Speichern");
				// Setzten des Titels und Styling 
				...
				saveBtn.addClickHandler(new ClickHandler() {
					...
					public void onClick(ClickEvent event) {
						...
						// Pruefung ob Name fuer die Kontaktliste angegeben wurde
						if (txtBox.getText() == "") {
